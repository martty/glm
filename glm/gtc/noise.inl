...
{
	template<typename T>
	GLM_FUNC_QUALIFIER vec<4, T> grad4(T const& j, vec<4, T> const& ip)
	{
		vec<3, T> pXYZ = floor(fract(vec<3, T>(j) * vec<3, T>(ip)) * T(7)) * ip[2] - T(1);
		T pW = static_cast<T>(1.5) - dot(abs(pXYZ), vec<3, T>(1));
		vec<4, T> s = vec<4, T>(lessThan(vec<4, T>(pXYZ, pW), vec<4, T>(0.0)));
	...
	template<typename T>
	GLM_FUNC_QUALIFIER T perlin(vec<2, T> const& Position)
	{
		...
		vec<4, T> gx = static_cast<T>(2) * glm::fract(i / T(41)) - T(1);
		...
		vec<4, T> norm = detail::taylorInvSqrt(vec<4, T>(dot(g00, g00), dot(g01, g01), dot(g10, g10), dot(g11, g11)));
		g00 *= norm.x;
		g01 *= norm.y;
		g10 *= norm.z;
		g11 *= norm.w;
	...
	// Classic Perlin noise
	template<typename T>
	GLM_FUNC_QUALIFIER T perlin(vec<3, T> const& Position)
	{
		vec<3, T> Pi0 = floor(Position); // Integer part for indexing
		vec<3, T> Pi1 = Pi0 + T(1); // Integer part + 1
		Pi0 = detail::mod289(Pi0);
		Pi1 = detail::mod289(Pi1);
		vec<3, T> Pf0 = fract(Position); // Fractional part for interpolation
		vec<3, T> Pf1 = Pf0 - T(1); // Fractional part - 1.0
		vec<4, T> ix(Pi0.x, Pi1.x, Pi0.x, Pi1.x);
		vec<4, T> iy = vec<4, T>(vec<2, T>(Pi0.y), vec<2, T>(Pi1.y));
		vec<4, T> iz0(Pi0.z);
		vec<4, T> iz1(Pi1.z);
		...
		vec<4, T> norm0 = detail::taylorInvSqrt(vec<4, T>(dot(g000, g000), dot(g010, g010), dot(g100, g100), dot(g110, g110)));
		g000 *= norm0.x;
		g010 *= norm0.y;
		g100 *= norm0.z;
		g110 *= norm0.w;
		vec<4, T> norm1 = detail::taylorInvSqrt(vec<4, T>(dot(g001, g001), dot(g011, g011), dot(g101, g101), dot(g111, g111)));
		g001 *= norm1.x;
		g011 *= norm1.y;
		g101 *= norm1.z;
		g111 *= norm1.w;
	...
	// Classic Perlin noise
	template<typename T>
	GLM_FUNC_QUALIFIER T perlin(vec<3, T> const& P)
	{
		...
		vec<4, T> ixy = permute(permute(ix) + iy);
		vec<4, T> ixy0 = permute(ixy + iz0);
		vec<4, T> ixy1 = permute(ixy + iz1);

		vec<4, T> gx0 = ixy0 / T(7);
		vec<4, T> gy0 = fract(floor(gx0) / T(7)) - T(0.5);
		gx0 = fract(gx0);
		vec<4, T> gz0 = vec<4, T>(0.5) - abs(gx0) - abs(gy0);
		vec<4, T> sz0 = step(gz0, vec<4, T>(0.0));
		...
		vec<4, T> gx1 = ixy1 / T(7);
		vec<4, T> gy1 = fract(floor(gx1) / T(7)) - T(0.5);
		gx1 = fract(gx1);
		vec<4, T> gz1 = vec<4, T>(0.5) - abs(gx1) - abs(gy1);
		vec<4, T> sz1 = step(gz1, vec<4, T>(0.0));
		gx1 -= sz1 * (step(T(0), gx1) - T(0.5));
		...
		vec<4, T> norm0 = taylorInvSqrt(vec<4, T>(dot(g000, g000), dot(g010, g010), dot(g100, g100), dot(g110, g110)));
		g000 *= norm0.x;
		g010 *= norm0.y;
		g100 *= norm0.z;
		g110 *= norm0.w;
		vec<4, T> norm1 = taylorInvSqrt(vec<4, T>(dot(g001, g001), dot(g011, g011), dot(g101, g101), dot(g111, g111)));
		g001 *= norm1.x;
		g011 *= norm1.y;
		g101 *= norm1.z;
		g111 *= norm1.w;
		...
		vec<3, T> fade_xyz = fade(Pf0);
		vec<4, T> n_z = mix(vec<4, T>(n000, n100, n010, n110), vec<4, T>(n001, n101, n011, n111), fade_xyz.z);
		vec<2, T> n_yz = mix(
			vec<2, T>(n_z.x, n_z.y),
			vec<2, T>(n_z.z, n_z.w), fade_xyz.y);
		T n_xyz = mix(n_yz.x, n_yz.y, fade_xyz.x);
		return T(2.2) * n_xyz;
	}
	*/
	// Classic Perlin noise
	template<typename T>
	GLM_FUNC_QUALIFIER T perlin(vec<4, T> const& Position)
	{
		vec<4, T> Pi0 = floor(Position);	// Integer part for indexing
		vec<4, T> Pi1 = Pi0 + T(1);		// Integer part + 1
		Pi0 = mod(Pi0, vec<4, T>(289));
		Pi1 = mod(Pi1, vec<4, T>(289));
		vec<4, T> Pf0 = fract(Position);	// Fractional part for interpolation
		vec<4, T> Pf1 = Pf0 - T(1);		// Fractional part - 1.0
		vec<4, T> ix(Pi0.x, Pi1.x, Pi0.x, Pi1.x);
		vec<4, T> iy(Pi0.y, Pi0.y, Pi1.y, Pi1.y);
		vec<4, T> iz0(Pi0.z);
		vec<4, T> iz1(Pi1.z);
		...
		vec<4, T> gw00 = vec<4, T>(0.75) - abs(gx00) - abs(gy00) - abs(gz00);
		...
		vec<4, T> gw01 = vec<4, T>(0.75) - abs(gx01) - abs(gy01) - abs(gz01);
		...
		vec<4, T> gw10 = vec<4, T>(0.75) - abs(gx10) - abs(gy10) - abs(gz10);
		...
		vec<4, T> gw11 = vec<4, T>(0.75) - abs(gx11) - abs(gy11) - abs(gz11);
		...
		vec<4, T> norm00 = detail::taylorInvSqrt(vec<4, T>(dot(g0000, g0000), dot(g0100, g0100), dot(g1000, g1000), dot(g1100, g1100)));
		g0000 *= norm00.x;
		g0100 *= norm00.y;
		g1000 *= norm00.z;
		g1100 *= norm00.w;

		vec<4, T> norm01 = detail::taylorInvSqrt(vec<4, T>(dot(g0001, g0001), dot(g0101, g0101), dot(g1001, g1001), dot(g1101, g1101)));
		g0001 *= norm01.x;
		g0101 *= norm01.y;
		g1001 *= norm01.z;
		g1101 *= norm01.w;

		vec<4, T> norm10 = detail::taylorInvSqrt(vec<4, T>(dot(g0010, g0010), dot(g0110, g0110), dot(g1010, g1010), dot(g1110, g1110)));
		g0010 *= norm10.x;
		g0110 *= norm10.y;
		g1010 *= norm10.z;
		g1110 *= norm10.w;

		vec<4, T> norm11 = detail::taylorInvSqrt(vec<4, T>(dot(g0011, g0011), dot(g0111, g0111), dot(g1011, g1011), dot(g1111, g1111)));
		g0011 *= norm11.x;
		g0111 *= norm11.y;
		g1011 *= norm11.z;
		g1111 *= norm11.w;
	...
	template<typename T>
	GLM_FUNC_QUALIFIER T perlin(vec<2, T> const& Position, vec<2, T> const& rep)
	{
		vec<4, T> Pi = floor(vec<4, T>(Position.x, Position.y, Position.x, Position.y)) + vec<4, T>(0.0, 0.0, 1.0, 1.0);
		vec<4, T> Pf = fract(vec<4, T>(Position.x, Position.y, Position.x, Position.y)) - vec<4, T>(0.0, 0.0, 1.0, 1.0);
		...
		vec<4, T> gy = abs(gx) - T(0.5);
		vec<4, T> tx = floor(gx + T(0.5));
		gx = gx - tx;
	...
	// Classic Perlin noise, periodic variant
	template<typename T>
	GLM_FUNC_QUALIFIER T perlin(vec<3, T> const& Position, vec<3, T> const& rep)
	{
		vec<3, T> Pi0 = mod(floor(Position), rep); // Integer part, modulo period
		vec<3, T> Pi1 = mod(Pi0 + vec<3, T>(T(1)), rep); // Integer part + 1, mod period
		Pi0 = mod(Pi0, vec<3, T>(289));
		Pi1 = mod(Pi1, vec<3, T>(289));
		vec<3, T> Pf0 = fract(Position); // Fractional part for interpolation
		vec<3, T> Pf1 = Pf0 - vec<3, T>(T(1)); // Fractional part - 1.0
		vec<4, T> ix = vec<4, T>(Pi0.x, Pi1.x, Pi0.x, Pi1.x);
		vec<4, T> iy = vec<4, T>(Pi0.y, Pi0.y, Pi1.y, Pi1.y);
		vec<4, T> iz0(Pi0.z);
		vec<4, T> iz1(Pi1.z);
		...
		gx1 = fract(gx1);
		vec<4, T> gz1 = vec<4, T>(0.5) - abs(gx1) - abs(gy1);
		vec<4, T> sz1 = step(gz1, vec<4, T>(T(0)));
		gx1 -= sz1 * (step(T(0), gx1) - T(0.5));
		gy1 -= sz1 * (step(T(0), gy1) - T(0.5));

		vec<3, T> g000 = vec<3, T>(gx0.x, gy0.x, gz0.x);
		vec<3, T> g100 = vec<3, T>(gx0.y, gy0.y, gz0.y);
		vec<3, T> g010 = vec<3, T>(gx0.z, gy0.z, gz0.z);
		vec<3, T> g110 = vec<3, T>(gx0.w, gy0.w, gz0.w);
		vec<3, T> g001 = vec<3, T>(gx1.x, gy1.x, gz1.x);
		vec<3, T> g101 = vec<3, T>(gx1.y, gy1.y, gz1.y);
		vec<3, T> g011 = vec<3, T>(gx1.z, gy1.z, gz1.z);
		vec<3, T> g111 = vec<3, T>(gx1.w, gy1.w, gz1.w);
	...
	template<typename T>
	GLM_FUNC_QUALIFIER T perlin(vec<4, T> const& Position, vec<4, T> const& rep)
	{
		vec<4, T> Pi0 = mod(floor(Position), rep); // Integer part modulo rep
		...
		gz11 = fract(gz11) - T(0.5);
		vec<4, T> gw11 = vec<4, T>(0.75) - abs(gx11) - abs(gy11) - abs(gz11);
		vec<4, T> sw11 = step(gw11, vec<4, T>(T(0)));
		gx11 -= sw11 * (step(T(0), gx11) - T(0.5));
		gy11 -= sw11 * (step(T(0), gy11) - T(0.5));
	...
	template<typename T>
	GLM_FUNC_QUALIFIER T simplex(glm::vec<2, T> const& v)
	{
		vec<4, T> const C = vec<4, T>(
		...
		// First corner
		vec<2, T> i  = floor(v + dot(v, vec<2, T>(C[1])));
		vec<2, T> x0 = v -   i + dot(i, vec<2, T>(C[0]));

		// Other corners
		//i1.x = step( x0.y, x0.x ); // x0.x > x0.y ? 1.0 : 0.0
		//i1.y = 1.0 - i1.x;
		vec<2, T> i1 = (x0.x > x0.y) ? vec<2, T>(1, 0) : vec<2, T>(0, 1);
		// x0 = x0 - 0.0 + 0.0 * C.xx ;
		// x1 = x0 - i1 + 1.0 * C.xx ;
		// x2 = x0 - 1.0 + 2.0 * C.xx ;
		...
		vec<3, T> p = detail::permute(
			detail::permute(i.y + vec<3, T>(T(0), i1.y, T(1)))
			+ i.x + vec<3, T>(T(0), i1.x, T(1)));

		vec<3, T> m = max(vec<3, T>(0.5) - vec<3, T>(
			dot(x0, x0),
		...
		vec<3, T> x = static_cast<T>(2) * fract(p * C.w) - T(1);
		vec<3, T> h = abs(x) - T(0.5);
		vec<3, T> ox = floor(x + T(0.5));
		vec<3, T> a0 = x - ox;
	...
	template<typename T>
	GLM_FUNC_QUALIFIER T simplex(vec<3, T> const& v)
	{
		vec<2, T> const C(1.0 / 6.0, 1.0 / 3.0);
		vec<4, T> const D(0.0, 0.5, 1.0, 2.0);
		...
		vec<4, T> j(p - T(49) * floor(p * ns.z * ns.z));  //  mod(p,7*7)

		vec<4, T> x_(floor(j * ns.z));
		vec<4, T> y_(floor(j - T(7) * x_));    // mod(j,N)

		vec<4, T> x(x_ * ns.x + ns.y);
		vec<4, T> y(y_ * ns.x + ns.y);
		vec<4, T> h(T(1) - abs(x) - abs(y));
		...
		vec<4, T> s0(floor(b0) * T(2) + T(1));
		vec<4, T> s1(floor(b1) * T(2) + T(1));
		vec<4, T> sh(-step(h, vec<4, T>(0.0)));
		...
		// Normalise gradients
		vec<4, T> norm = detail::taylorInvSqrt(vec<4, T>(dot(p0, p0), dot(p1, p1), dot(p2, p2), dot(p3, p3)));
		p0 *= norm.x;
		p1 *= norm.y;
		p2 *= norm.z;
		p3 *= norm.w;
	...
	template<typename T>
	GLM_FUNC_QUALIFIER T simplex(vec<4, T> const& v)
	{
		vec<4, T> const C(
		...
		vec<4, T> i0;
		vec<3, T> isX = step(vec<3, T>(x0.y, x0.z, x0.w), vec<3, T>(x0.x));
		vec<3, T> isYZ = step(vec<3, T>(x0.z, x0.w, x0.w), vec<3, T>(x0.y, x0.y, x0.z));
		//  i0.x = dot(isX, vec3(1.0));
		//i0.x = isX.x + isX.y + isX.z;
		//i0.yzw = static_cast<T>(1) - isX;
		i0 = vec<4, T>(isX.x + isX.y + isX.z, T(1) - isX);
		//  i0.y += dot(isYZ.xy, vec2(1.0));
		i0.y += isYZ.x + isYZ.y;
		//i0.zw += 1.0 - vec<2, T>(isYZ.x, isYZ.y);
		...
		vec<4, T> i3 = clamp(i0, T(0), T(1));
		vec<4, T> i2 = clamp(i0 - T(1), T(0), T(1));
		vec<4, T> i1 = clamp(i0 - T(2), T(0), T(1));
		...
		vec<4, T> ip = vec<4, T>(T(1) / T(294), T(1) / T(49), T(1) / T(7), T(0));

		vec<4, T> p0 = gtc::grad4(j0,   ip);
		vec<4, T> p1 = gtc::grad4(j1.x, ip);
		vec<4, T> p2 = gtc::grad4(j1.y, ip);
		vec<4, T> p3 = gtc::grad4(j1.z, ip);
		vec<4, T> p4 = gtc::grad4(j1.w, ip);
